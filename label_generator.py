...
print(f"Labels: {names_dict}")
print(f"Total labels:{len(names_dict)}")
...
try:
    ...
    print(f"Total images: {image_counter} images.")
...
# data preprocessing for label
data_df = pd.read_csv(r"D:\clean_dataset\label.csv")
print(f"label.csv shape: {data_df.shape}") # should return (total of images, 3)

#drop id from the table
data_df.drop(["id"],axis='columns', inplace=True)

# create one-hot-encoding based on label
data_df_encoded = pd.get_dummies(data_df,columns=["label"],dtype=float)

# add label back into encoded_df
data_df_encoded["label"] = data_df["label"]

# verify the result
print(f"face_label_encoded.csv shape: {data_df_encoded.shape}") # should return (total of images, total of label + 2)

#print(data_df_encoded.head(2))
#print(data_df_encoded.tail(2))

# save the result
data_df_encoded.to_csv(os.path.join(faces_folder, 'face_label_encoded.csv'))